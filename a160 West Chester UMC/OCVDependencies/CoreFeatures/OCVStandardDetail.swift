...
protocol StandardDetailDisplayable {
    var title: String { get }
    var detailContent: String { get }
    var htmlContent: String? { get }
    var dateString: String { get }
    var images: [String]? { get }
}

class OCVStandardDetail: UIViewController {

    let detailTitle: String!
    let detailDesc: String!
    let detailHTMLContent: String!
    ...
    init(object: StandardDetailDisplayable, inWebview: Bool) {
        detailTitle = object.title
        detailDesc = object.detailContent
        detailHTMLContent = object.htmlContent
        detailDate = object.dateString
        detailImages = object.images
        self.runWebview = inWebview
        super.init(nibName: nil, bundle: nil)
    }

    convenience init(object: StandardDetailDisplayable) {
        self.init(object: object, inWebview: false)
    }
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = AppColors.standardWhite.color
         ...
         self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .action, target: self, action: #selector(OCVStandardDetail.share))
    ...
    func configureDetailView() {
        ...
        if let html = detailHTMLContent {
            ...
            detailTextLabel.text = self.detailDesc
        }
        ...
        fontSlider.addTarget(self, action: #selector(OCVStandardDetail.changeFontToSliderValue), for: UIControlEvents.valueChanged)
        ...
        guard let images = detailImages else {
        ...
        if !images.isEmpty {
            ...
            for image in images {
                let imageURL = URL(string: image) ?? URL(string: "")

                let imageButtonView = UIView(frame: CGRect(x: 0, y: 0, width: 135, height: 135))

                let imageButton: CustomImageButton = {
                    $0.backgroundColor = UIColor.clear
                    $0.addTarget(self, action: #selector(OCVStandardDetail.openWebviewToImage(_: )), for: .touchUpInside)
                    return $0
                }(CustomImageButton(frame: imageButtonView.frame, urlToOpen: image))

                let newImageView: UIImageView = {
                    $0.frame = CGRect(x: 0, y: 0, width: 135, height: 135)
                    $0.contentMode = .scaleAspectFill
                    $0.clipsToBounds = true
                    $0.af_setImage(withURL: imageURL!, placeholderImage: UIImage(named: "logo"))
                    return $0
                }(UIImageView())
    ...
    func setupDetailAsWebview() {
        ...
        if let htmlString = detailHTMLContent {
    ...
    func changeFontToSliderValue() {
        ...
        detailTextLabel.font = AppFonts.RegularText.font(fontSlider.value)
    ...
    func toolBarItemsWithDate() -> [UIBarButtonItem] {
        ...
        let fontButton = UIBarButtonItem(image: UIImage(named: "fontSize"), style: .plain, target: self, action: #selector(OCVStandardDetail.showSlider))
    ...
    func toolBarItemsWithSlider() -> [UIBarButtonItem] {
        let dateButton = UIBarButtonItem(image: UIImage(named: "dateTimeClock"), style: .plain, target: self, action: #selector(OCVStandardDetail.showDate))
        let flex = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: nil)
